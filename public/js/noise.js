...
function create_chart(xlabels, ytemps, period) {
    ...
    gradient.addColorStop(0.15, '#F46B41');
    gradient.addColorStop(0.40, '#90FC4E');
    gradient.addColorStop(0.50, '#43CFEA');
...
function get_data(index) {
    ...
    xhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            json.forEach(element => {
                ...
                ytemps.push(Math.round(element.db * 10) / 10);
            });
    ...
    xhttp.open("GET", "/db_noise" + querry, true);
    xhttp.send();
}
